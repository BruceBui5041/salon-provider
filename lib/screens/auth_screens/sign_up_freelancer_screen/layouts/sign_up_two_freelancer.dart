    ...
    return Consumer<SignUpCompanyProvider>(builder: (context, value, child) {
      return Column(children: [
        ...
        ContainerWithTextLayout(title: appFonts.country)
            .paddingOnly(bottom: Insets.i8),
        StateCountryDropdown(items: value.countryList,selectedItem: value.countryCompany,onChanged: (val) => value.onChangeCountryCompany(val))
            .paddingSymmetric(horizontal: Insets.i20),
        ...
        ContainerWithTextLayout(title: appFonts.state)
            .paddingOnly(bottom: Insets.i8),
        StateCountryDropdown(icon: eSvgAssets.state,items: value.stateList,selectedItem: value.stateCompany,onChanged: (val) => value.onChangeStateCompany(val))
            .paddingSymmetric(horizontal: Insets.i20),
        ...
        SliderLayout(val: value.slider,onDragging: (handlerIndex, lowerValue, upperValue) =>
            value.slidingValue(lowerValue)).padding(horizontal: Insets.i8,bottom: Insets.i10).boxShapeExtension(color: appColor(context).appTheme.whiteBg).paddingSymmetric(horizontal: Insets.i20),
        const VSpace(Sizes.s20),
          ...
          Column(
              children: [
                Row(
                    ...
                    children: [
                      Text(language(context, appFonts.serviceAvailableArea), style: appCss.dmDenseMedium14
                          .textColor(appColor(context).appTheme.lightText)),
                      Text(language(context, "+${language(context, appFonts.add)}"), style: appCss.dmDenseMedium14
                          .textColor(appColor(context).appTheme.primary)).inkWell(onTap: ()=> route.pushNamed(context, routeName.location))
                    ]
                ).paddingSymmetric(horizontal: Insets.i20,vertical: Insets.i10),
                ...appArray.serviceAvailableAreaList.asMap().entries.map((e) => ServicemanListLayout(
                    data: e.value,
                    onDelete: ()=> value.onLocationDelete(e.key,context,sync),
                    index: e.key,
                    list: appArray.serviceAvailableAreaList
                ).paddingSymmetric(horizontal: Insets.i20)).toList()
              ]
          ),
        const VSpace(Sizes.s30),
        const DottedLines(),
