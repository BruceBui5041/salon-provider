...
class _LocationListScreenState extends State<LocationListScreen> with TickerProviderStateMixin {
  @override
      ...
      builder: (context,value,child) {
        return Scaffold(
          appBar: AppBar(
              ...
              actions: [
                CommonArrow(arrow: eSvgAssets.add,onTap: ()=> route.pushNamed(context, routeName.location)).paddingOnly(right: Insets.i20)
              ]),
          body: Column(
            ...
            children: [
              Column(
                children: [
                  ...value.locationList.asMap().entries.map((e) => ServicemanListLayout(
                      isCheck: value.selectedLocation.contains(e.key),
                      onIconTap: ()=> value.onTapLocation(e.key,e.value),
                      isBorder: true,
                      onEdit: ()=> value.onEditLocation(e.key,e.value,context),
                      onDelete: ()=> value.onLocationDelete(e.key,context,this),
                      data: e.value,
                      index: e.key,
