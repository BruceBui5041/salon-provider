...
class CompletedBookingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer2<CompletedBookingProvider,AddServiceProofProvider>(builder: (context, value,serviceProofVal, child) {
      return StatefulWrapper(
          ...
          child: Scaffold(
              ...
              body: Stack(alignment: Alignment.bottomCenter, children: [
                          ...
                          children: [
                            ...
                            Row(
                                ...
                                children: [
                                  SizedBox(
                                      width: Sizes.s200,
                                      child: Text(language(context,  appFonts.serviceProof),overflow: TextOverflow.ellipsis,style: appCss.dmDenseBold18.textColor(appColor(context).appTheme.darkText))),
                                  Text(language(context,  appFonts.editProof),style: appCss.dmDenseRegular14.textColor(appColor(context).appTheme.primary)).inkWell(onTap: ()=> route.pushNamed(context, routeName.addServiceProof))
                                ]
                            ),
                            if(serviceProofVal.proofList.isNotEmpty)
                              SingleChildScrollView(
                               ...
                               child: Row(
                                 children: serviceProofVal.proofList.asMap().entries.map((e) => Container(
                                     height: Sizes.s70,
                                     width: Sizes.s70,
                                     decoration: ShapeDecoration(
                                         ...
                                         shape: RoundedRectangleBorder(
                                             borderRadius: SmoothBorderRadius(
                                                 ...
                                                 cornerSmoothing: 1)))).paddingOnly(right: e.key != serviceProofVal.proofList.length -1 ? Insets.i10 : 0)).toList()
                               ).paddingAll(Insets.i15).boxBorderExtension(context,bColor: appColor(context).appTheme.stroke)
                ...
                Material(
                    elevation: 20,
                    child: serviceProofVal.proofList.isNotEmpty ? ButtonCommon(
                          onTap: ()=> route.pushNamed(context, routeName.addServiceProof),
                          title: appFonts.addServiceProof,
                        style: appCss.dmDenseRegular16
                            .textColor(appColor(context).appTheme.primary),
                        color: appColor(context).appTheme.trans,
                        borderColor: appColor(context).appTheme.primary).paddingAll(Insets.i20)
                        .decorated(color: appColor(context).appTheme.whiteBg) : ButtonCommon(
                      onTap: ()=> route.pushNamed(context, routeName.addServiceProof),
                        title: appFonts.addServiceProof)
                        .paddingAll(Insets.i20)
