...
class ServiceRangeLayout extends StatelessWidget {
  ...
  final int? index;
  final List? list;
  const ServiceRangeLayout({super.key,this.data,this.onToggle,this.list,this.index, this.onDelete, this.onEdit});
        ...
        Column(
            children: [
              ListTile(
                  ...
                  title: Row(children: [
                    Text(language(context, "${data["street"]} - ${data["city"]}"),
                        overflow: TextOverflow.ellipsis,
                        style: appCss.dmDenseMedium12
                            .textColor(appColor(context).appTheme.darkText)).width(Sizes.s110)
                  ]),
                  subtitle: Text(language(context, "${data["state"]} - ${data["zip_code"]}" ),
                      style: appCss.dmDenseMedium12
              ...
              Row(
                  ...
                  children: [
                    Text(language(context, appFonts.serviceAvailable),style: appCss.dmDenseRegular12.textColor(appColor(context).appTheme.darkText)),
                    FlutterSwitchCommon(value: data["status"],onToggle: onToggle)
                  ]
              ).paddingOnly(bottom: Insets.i15)
            ]
        ).paddingSymmetric(horizontal: Insets.i15).
        boxShapeExtension(color: appColor(context).appTheme.fieldCardBg).paddingOnly(bottom: index != list!.length -1 ? Insets.i15 : 0 )
      ]
    );
  }
}
